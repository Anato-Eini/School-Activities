    ...
    $.ajax({
        ...
        success: (data)=>{
            ...
            JSON.parse(data).forEach(elements => {
                if(elements.organizer === username){
                    $('<div class="event"><span class="eventPost" id="' + ("eventID" + eventID) + '">' +
                        '<h2>' + elements.eventName + '<span class="closeIcon" onclick="notifyAll(\''+ elements.eventName +
                        '\', prompt(\'State your reason: \')); cancelEvent(\''+ elements.eventName +'\')">' +
                    ...
                    elements.reviews.forEach(review => {
                        $('<div class="review"><h4>' + review.username + '<span class="closeIcon" ' +
                            'onclick="deleteReview(\'' + elements.eventName + '\', \'' + username + '\', \'' + elements.review + '\')">' +
                    ...
                    $('<div class="event"><span class="eventPost" id="' + ("eventID" + eventID) + '">' +
                        ...
                        '</div><div class="participants" id="'+ ("participantsID" + (eventID)) + '">Participants:</div>' +
                        '<div class="reviews" id="'+ ("reviewID" + eventID++)+'"><h3>Reviews</h3></div>'+
                        '</div>').prependTo('#containerOne')
                    elements.reviews.forEach(review => {
